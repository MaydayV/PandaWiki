...
## 追加记录（2026-03-07，上游 1-5 选择性合并与功能增强）
- 版本升级：
  - 乘风版版本号升级为：`FV2.6.16.2111`。
- 上游能力合并（按乘风版方向选择性吸收）：
  - 新增后端文件 URL 上传接口（管理端与分享端），并补齐 SSRF 防护、重定向拦截与远端文件大小限制。
  - 编辑器支持“粘贴图片链接自动上传转存”（管理端与前台）。
  - 首页 Header 搜索框交互优化（随滚动动态显示）。
  - 修复前端 workspace `dev` 脚本过滤器配置。
- 模型配置增强：
